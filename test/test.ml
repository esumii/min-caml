...
(*MINCAML*) let ttrue = 1 in
(*MINCAML*) let ffalse = 0 in
...
(* 曲面の外側が真かどうかのフラグ ttrue:外側が真 ffalse:内側が真 *)
...
(* 回転の有無 ttrue:回転あり ffalse:回転無し 2次曲面と円錐のみ有効 *)
let rec o_isrot m =
...
let rec read_nth_object n =
  ...
  if texture <> -1 then
    (
      ...
      (* 注: 下記正規化 (form = 2) 参照 *)
      let m_invert2 = if form = 2 then ttrue else m_invert in
...
let rec solver_rect_surface m dirvec b0 b1 b2 i0 i1 i2  =
  if fiszero dirvec.(i0) then ffalse else
...
let rec shadow_check_and_group iand_ofs and_group =
  if and_group.(iand_ofs) = -1 then
    ...
    if (if t0 <> 0 then fless t0p (-0.2) else ffalse) then
  ...
  if nref <= 4 then (
    ...
    if judge_intersection dirvec then (
      ...
      if fless (o_diffuse obj) 0.5 then
        calc_diffuse.(nref) <- ffalse
      else (
        calc_diffuse.(nref) <- ttrue;
...
let rec pretrace_pixels line x group_id lc0 lc1 lc2 =
  if x >= 0 then (
    ...
    vecunit_sgn ptrace_dirvec ffalse;
    vecbzero rgb;
...
let rec create_pixel _ =
  ...
  let m_cdif = create_array 5 ffalse in
